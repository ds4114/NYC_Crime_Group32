...
We start our analysis of crime types by investigating which crime categories occur in what frequencies. We bucketed the numerous crimes that are listed into logical categories. Note that in the pre-processing stage, we removed a small "Other" category of parking violations and noise complaints. 
...
--TODO, do we need 3 charts above that show the same thing?


### Counts of crime by borough
...
vcd::mosaic(VIC_AGE_GROUP ~ BORO_NM + Inside_Outside, counts3, direction = c("v","v", "h"))
```
...
### Spatial distribution of all crime around Columbia University
...
### Histogram of crime frequency with respect to increasing radius from Columbia
...
### Cumulative crime counts with respect to distance from Columbia University
...
### Cumulative and incremental density of crime with respect to distance from Columbia
...
### Visualizing incremental density of crime within 1000m of Columbia
...
===========================================
## Various Other Plots (To Revise/Delete)
--ATTEMPT AT MOSAIC PLOT 
```{r, echo=FALSE, eval=FALSE}
counts2 <- df_key_fields %>%
  group_by(VIC_RACE_short, SUSP_RACE_short) %>%
  summarize(Freq = n())
vcd::mosaic(VIC_RACE_short ~ SUSP_RACE_short, counts2, direction = c("v", "h"),)
```
